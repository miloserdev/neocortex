...
int xdp_ddos_detect ( struct xdp_md *ctx ) {
	...
	if ( entry )
	{
		bpf_printk ( "entry: then goto __pass ??? " );
		// ???
		// goto __pass;
	}
...
__process:
	...
	if (
		...
		/* cannot allocate event */
		goto __drop;    //	???
	}

	event->ip = ip->saddr;
	// bpf_probe_read(event->data, payload_len, payload);
	/*
		for (size_t i = 0; i < DATA_RAW_SIZE; i++) {
			if (data + i + 1 > data_end) break;
			event->data[i] = *((__u8 *)(data + i));
		}
	 */
